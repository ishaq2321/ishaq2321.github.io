...
async function main() {
  ...
  console.log(`GitHub stats → public/github-stats.json (${stats.repos} repos, ${stats.stars} stars)`);
}
...
main().catch((err) => {
  ...
  writeFileSync(
    ...
    JSON.stringify({ repos: 0, followers: 0, stars: 0, updatedAt: new Date().toISOString() }, null, 2),
  );
